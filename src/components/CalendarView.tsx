...
const CalendarView = ({ jobs, onSelectJob }: CalendarViewProps) => {
  ...
  const allItems = useMemo(() => {
    ...
    for (const job of jobs) {
      for (const evt of job.events ?? []) {
        items.push({ kind: "event", data: { jobId: job.id, company: job.company, role: job.role, event: evt } });
      }
      if (job.closeDate) {
        items.push({ kind: "deadline", data: { jobId: job.id, company: job.company, role: job.role, date: job.closeDate } });
      }
    }
    return items;
  }, [jobs]);
  ...
  const monthItems = useMemo(() => {
    return allItems
      .filter((item) => {
        const d = item.kind === "event" ? item.data.event.date : item.data.date;
        try { return isSameMonth(parseISO(d), month); } catch { return false; }
      })
      .sort((a, b) => {
        const dateA = a.kind === "event" ? a.data.event.date : a.data.date;
        const dateB = b.kind === "event" ? b.data.event.date : b.data.date;
  ...
  const displayLabel = selectedDate ? format(selectedDate, "EEEE, MMMM d") : format(month, "MMMM yyyy");

  const modifiers = useMemo(() => {
    const hasEvent: Date[] = [];
    eventDates.forEach((d) => { try { hasEvent.push(parseISO(d)); } catch { /* skip */ } });
    return { hasEvent };
  }, [eventDates]);

  const modifiersStyles = { hasEvent: {} as React.CSSProperties };

  const renderEventList = () => (
    <>
      <div className="flex items-center gap-2 border-b border-border px-4 sm:px-6 py-4">
            ...
            displayItems.map((item, i) => {
              if (item.kind === "event") {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className={cn("text-[10px] border", TYPE_STYLES[evt.type])}>
                    ...
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Briefcase className="h-3 w-3" /> {item.data.company} — {item.data.role}
                      </span>
                      {evt.time && (
                    ...
                    {isPast && !evt.outcome && (
                      <p className="text-[10px] text-accent font-medium">Outcome not recorded</p>
                    )}
                  </div>
                );
              } else {
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className={cn("text-[10px] border", TYPE_STYLES.deadline)}>Deadline</Badge>
