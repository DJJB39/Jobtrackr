...
export const useInterviewCoach = (
  ...
  const [bootcampContext, setBootcampContext] = useState<any>(null);
    ...
    async (answerText?: string) => {
      ...
      const feedbackText = await stream(
        AI_URL,
        {
          ...
          bootcampContext: bootcampContext || undefined,
        },
        token,
  ...
  return {
    ...
    startBootcampRoast: (selectedJob: JobApplication, bootcampData: { company_snapshot: { why_join: string; recent_news: string; product_context: string }; questions: Array<{ question: string; type: string; context_note: string }> }) => {
      ...
      setBootcampContext({
        company_snapshot: bootcampData.company_snapshot.why_join,
        recent_news: bootcampData.company_snapshot.recent_news,
        product_context: bootcampData.company_snapshot.product_context,
      });
      // We start a normal ruthless session — questions will be generated by AI but with bootcamp context
      startSession(selectedJob, "ruthless");
    },
    speakQuestion,
    startListening,
    stopListening,
